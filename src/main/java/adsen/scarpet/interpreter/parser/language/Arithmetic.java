...
public class Arithmetic {
    public static void apply(Expression expression) {
        ...
        expression.addMathematicalUnaryFunction("bitwise_not", (d) -> (double) (~d.longValue()));
        ...
        expression.addMathematicalUnaryFunction("atanh", (d) -> {                                  // Formula: atanh(x) = 0.5*ln((1 + x)/(1 - x))
            if (Math.abs(d) > 1 || Math.abs(d) == 1)
                throw new InternalExpressionException("Number must be -1 < x < 1");
        ...
        expression.addMathematicalUnaryFunction("exp", Math::exp);
        ...
        expression.addMathematicalUnaryFunction("sigmoid", (d) -> 1 / (1 + Math.exp(-d)));
        expression.addMathematicalUnaryFunction("fast_sigmoid", (d) -> d / (2 + 2 * Math.abs(d)) + 0.5D);
